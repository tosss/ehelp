...
11/15/16    Venkata Shrivol     Added code in beforeDelete context to prevent deletion of RMS sites RQ-07985 
12/14/16    Venkata Shrivol		commented the un-used discovery code and project object code RQ-08305
------------------------------------------------------------*/
trigger IMP_SubProjectSite on SubProjectSite__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {
    ...
    if (Trigger.isBefore) {
        ...
        {
            for(SubProjectSite__c sps:Trigger.old)
            {
                if(sps.RMS_Request_Ids__c!=null && sps.RMS_Request_Ids__c != '')
                {
                    sps.addError(' SubProjectSite with RMS Request Ids cannot be deleted');
                }
            }
