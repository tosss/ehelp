({
    ...
    sort : function(component, event, helper)
    {
        ...
        if(colName != "Products_of_Interest__c" && colName !="MSR_Notes__c" && colName !="Phone")
        {
            if(colName == existingSort)
            {
                jQuery(event.target).parents('th').find('.sortImage').toggle();
                var sortingOrder = component.get("v.ascending");
                component.set("v.ascending",!sortingOrder);
            }
            else
            {
                ...
                component.set("v.orderBy",colName);
