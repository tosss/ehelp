trigger DS_NADASwipeDeviceLookup on NADA_Swipe__c (before insert, before update)
{
    ...
    for(NADA_Swipe__c s : Trigger.New)
    {
        ...
        if(s.Phone__c != null) { 
            ...
            if(pm.matches()) { s.Phone__c = '('+pm.group(1)+') '+pm.group(2) + '-' + pm.group(3); }
        }
        
        if(s.Fax__c != null) { 
            Matcher fm = FormatPhoneFax.matcher(s.Fax__c);
            if(fm.matches()) { s.Fax__c = '(' + fm.group(1)+') ' + fm.group(2) + '-' + fm.group(3); }
        }
        // finish formatting
        ...
        if(Trigger.isInsert) { 
        	CurrentSwipes.add(s); AllApparatusIds.add(s.Apparatus_Id__c); SwipeCardNumbers.add(s.Swiped_Card_Number__c); }
    }
    ...
    if(!Devices.IsEmpty()) 
    {
        for(NADA_Device__c n : Devices) { DevicesByApparatusIds.put(n.Name, n); }
    }
